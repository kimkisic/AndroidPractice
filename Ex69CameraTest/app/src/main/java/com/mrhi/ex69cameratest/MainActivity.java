package com.mrhi.ex69cameratest;
        ...
        switch ( requestCode ){
            case 100:
                ...
                if(resultCode == RESULT_OK){
                    ...
                    //결과물을 가져온 Intent객체(3번째 파미미터: data)에게 결과 받기
                    ...
                    if( uri != null ){ //파일로 저장되는 방식일 때
                        ...
                        //이런 작업을 자동으로 해주는 라이러리 사용(Glide, Picasso)
                        //iv.setImageURI(uri);
                        Toast.makeText(this, "uri로 사진정보 획득", Toast.LENGTH_SHORT).show();
                        Glide.with(this).load(uri).into(iv);

                    }else{//Bitmap이미지 데이터만 줄때[요즘은 거의 이방식]
                        Toast.makeText(this, "Bitmap객체로 사진정보 획득", Toast.LENGTH_SHORT).show();

                        //Bitmap객체를 Intent로 부터 Extra데이터로 전달 받음
